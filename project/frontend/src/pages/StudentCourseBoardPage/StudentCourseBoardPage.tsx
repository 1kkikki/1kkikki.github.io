...
import { getBoardPosts, createBoardPost, deleteBoardPost, updateBoardPost, getComments, createComment, deleteComment, toggleLike, toggleCommentLike, uploadFile, votePoll, togglePinPost } from "../../api/board";
...
export default function CourseBoardPage({ course, onBack, onNavigate, availableTimes = [] }: CourseBoardPageProps) {
  ...
  async function loadPosts() {
    try {
      ...
      // 디버깅: 팀 게시판 게시글 확인
      const teamPosts = mapped.filter(p => p.category === "팀 게시판");
      if (teamPosts.length > 0) {
        console.log("[DEBUG] 팀 게시판 게시글 목록:", teamPosts.map(p => ({
          id: p.id,
          title: p.title,
          category: p.category,
          team_board_name: p.team_board_name,
          author: p.author,
          author_id: p.author_id
        })));
      }
