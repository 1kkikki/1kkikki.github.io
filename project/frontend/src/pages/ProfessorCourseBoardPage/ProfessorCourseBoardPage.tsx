...
export default function CourseBoardPage({ course, onBack, onNavigate }: CourseBoardPageProps) {
  ...
  useEffect(() => {
    ...
    // 게시글을 찾지 못한 경우
    if (!targetPost) {
      // posts가 로드되었지만 해당 게시글이 없는 경우에만 정리
      // (탭 전환 직후 posts가 아직 업데이트 안 된 경우 대기)
      return; // posts가 업데이트되면 다시 시도
    }
  ...
  }, [notificationTargetPostId, posts, course.code, loadedCourseCode, activeTab]); // activeTab 추가 // loadedCourseCode 추가 - posts 로딩 완료 확인
